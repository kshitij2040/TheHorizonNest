...
const FloatingDockDesktop = ({
  ...
  return (
    (<motion.div
      ...
      className={cn(
        "mx-auto hidden md:flex h-16 gap-4 items-end  rounded-2xl bg-slate-950 px-4 pb-3",
        className
      )}>
      {items.map((item) => (
        ...
        <AnimatePresence>
          {hovered && (
            <motion.div
              ...
              className="px-2 py-0.5 whitespace-pre rounded-md bg-neutral-800 dark:border-neutral-900 text-white border-gray-200  absolute left-1/2 -translate-x-1/2 -top-8 w-fit text-xs">
              {title}
            </motion.div>
          )}
        </AnimatePresence>
